...
- [../devel/capture_real_gameplay_audio.mjs](../devel/capture_real_gameplay_audio.mjs) records
  one-time post-master A/V and analyser evidence from live real-worker rolls. It is a developer
  probe, not a permanent browser test; its media and reports belong in ignored `_temp/` output.
...
- Add sound playback under [../src/audio/](../src/audio/) and physical-to-audio mapping under
  [../src/app/impact_presentation.ts](../src/app/impact_presentation.ts). Add a permissively
  licensed committed clip under [../src/assets/audio/](../src/assets/audio/) with its matching
  provenance entry in [../src/assets/audio/LICENSES.md](../src/assets/audio/LICENSES.md).
...
- Complete a human listening and integrated A/V review of full ten-pin and 990-pin rolls. The
  one-time capture probe establishes reproducible evidence, but it cannot itself establish the
  perceived hierarchy of rolling, impacts, cascade, result, camera, celebration, and pacing.
