...
| Lane geometry      | The lane uses foot-based regulation proportions: fixed 60 ft ball travel and fixed 9.25 in gutters, while lane and deck width scale with the complete triangular rack. The front-facing projection keeps a playable full lane and recognizable fixed board vocabulary at every rack size. |
...
| Falling pins       | One displacement-or-impulse threshold emits one fall event. Deadwood is removed into the pit between rolls on the same rack, so the next ball sees only standing pins; the renderer omits removed or in-pit pins.                                                                         |
...
| M9: Regulation lane rebuild           | Delivered | Foot-based lane physics, gutter and pit cleanup, four pre-roll bowling controls, worker-backed preview, updated ball, pin, and lane presentation, and maintained 1600 x 1000 captures are present. The 30-shot gate settled without false timeouts; the 990 median was 431.27 ms, within the 750 ms budget. Current Playwright specs pass in focused/component reruns; two 23/24 full-suite attempts were interrupted by host suspension or browser session closure. |
