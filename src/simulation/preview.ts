...
import { normalize_aim } from "../game/aim";
...
export async function create_preview_path(
  ...
  const normalized_launch = normalize_aim(pin_count, launch);
  const world = new RAPIER.World({ x: 0, y: 0 });
  world.timestep = physics_config.fixed_step_seconds;
  create_preview_lane(world, pin_count);
  const ball = world.createRigidBody(
    RAPIER.RigidBodyDesc.dynamic()
      .setTranslation(normalized_launch.start_position, 0)
      .setLinearDamping(0)
      .lockRotations(),
  );
  ...
  ball.setLinvel(
    {
      x: Math.sin(normalized_launch.angle) * normalized_launch.power,
      y: Math.cos(normalized_launch.angle) * normalized_launch.power,
    },
    true,
  );
  let state: BallForceState = {
    spin: normalized_launch.spin,
    in_gutter: false,
    in_pit: false,
    has_hit_pin: false,
    ...
    last_y: 0,
    launch_direction: {
      x: Math.sin(normalized_launch.angle),
      y: Math.cos(normalized_launch.angle),
    },
  };
  const points: number[] = [];
