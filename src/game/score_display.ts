...
export function format_frame_roll_marks(frame: FrameScore, pin_count: RackPinCount): string[] {
  if (is_strike(frame, pin_count)) return ["X"];
  return frame.rolls.map((roll, roll_index) => {
    if (roll === pin_count) return "X";
    const first_roll = frame.rolls[0];
    if (roll_index === 1 && first_roll !== pin_count && is_spare(frame, pin_count)) return "/";
    return String(roll);
  });
}
